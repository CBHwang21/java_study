...
public class Market {
	...
	public void genProduct() {
		...
		grain[1] = new Grain("곡류", "잡곡", 8000, 13);
	...
	public void selectEvent(Event event) {
		...
		random1 = Sale.getRandomDiscount();
		random2 = Sale.getRandomProduct();
		System.out.println("# 오늘의 행사상품 : " + random2 + " " + random1 + "원 할인쿠폰증정");
	...
	public void selectProduct(Product[] products) {
		...
		if (productNo == 0) {
			...
			checkout();
		} else if (productNo == 1 || productNo == 2 || productNo == 3 || productNo == 4 || productNo == 5
				|| productNo == 6 || productNo == 7 || productNo == 8 || productNo == 9 || productNo == 10
				|| productNo == 11 || productNo == 12) {
	...
	public void printShoppingBasket() {
		...
		// random1 : 행사상품 할인가격, random2 : 행사상품
		// 장바구니 목록에 행사상품이 있으면 행사상품만 할인가격만큼 빼고 나머지는 그대로 계산		
		for (int i = 0; i < cart.size(); i++) {
			cart.get(i).getProductName();
			cart.get(i).getPrice();
			String gPN = cart.get(i).getProductName();
			int gP = cart.get(i).getPrice();
			if (gPN.contains(random2)) {
				System.out.printf(" %d   ┃\t%s\t   \t┃%d \n", i + 1, random2, gP - random1);
			} else
				System.out.printf(" %d   ┃\t%s\t   \t┃%d \n", i + 1, gPN, gP);
		}
